"""Colour maps: the catalogue, and the base every map derives from.

`_show` decides whether `colors=` is a colormap with `isinstance(colors,
BaseColorMap)`, so there can be one base class rather than one per host - two
identical definitions is exactly what `isinstance` cannot see through, and it
fails silently, taking the colormap for a list of colours.

The catalogue is here for the same reason it is worth sharing at all: every
client wants the same `tab10`. It needs colorsys, random and webcolors, and
uses matplotlib's colour maps when matplotlib happens to be installed, so it
costs a host nothing to have.
"""
...
# Anything at all: matplotlib is optional here, and a broken installation of it
# should leave the built-in colour maps working rather than fail the import.
...
def matplotlib_mapper(t, name):
    ...
    if not isinstance(colormap, mpl.colors.LinearSegmentedColormap):
        # ValueError rather than TypeError: the caller passed a name, and the
        # name is what is wrong - the type it happens to resolve to is this
...
class ColorMap:
    ...
    def listed(length=10, colormap="mpl:plasma", colors=None, alpha=1.0, reverse=False):
        ...
        if colors is None:
            ...
            # `ListedColormap.colors` is annotated as anything array-like, so
            # neither len() nor subscripting types - but the isinstance check
            # above has already established that this is a listed colormap, and
            # a listed one carries a sequence.
            listed = list(colormap.colors)  # ty: ignore[invalid-argument-type]
            interval = len(listed) // (length - 1)
            colors = [listed[i] for i in range(0, len(listed), interval)]
